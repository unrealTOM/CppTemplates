...
typename std::enable_if<
    returns_void<OnSuccess(InputType)>::value, void
>::type
api_exec(Function func, OnSuccess on_success, OnError on_error, InputType input, Args... args) {
    ...
    static_assert(std::is_integral<ReturnType>::value, "Please only call me with integral types! 1");
...
api_exec(Function func, OnSuccess on_success, OnError on_error, Args... args) {
    ...
    static_assert(std::is_integral<ReturnType>::value, "Please only call me with integral types! 2");
...
void api_exec(Function func, Args... args) {
    typedef typename std::result_of<Function(Args...)>::type ReturnType;
    static_assert(std::is_integral<ReturnType>::value, "Please only call me with integral types! 3");
...
template<
    ...
    typename OnSuccess,
    typename OnError,
    typename = int, //InputType,
    typename... Args>
typename std::enable_if<
    returns_void<OnSuccess(int)>::value
    >::type
api_exec(Function func, OnSuccess on_success, OnError on_error, int input, Args... args) {
    typedef typename std::result_of<Function(Args...)>::type ReturnType;
    static_assert(std::is_integral<ReturnType>::value, "Please only call me with integral types! 4");
